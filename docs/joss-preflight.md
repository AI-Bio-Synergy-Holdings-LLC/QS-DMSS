...
Prepare the packet, collect reviewer feedback, and use `v0.5.0` as the stable
public baseline for Lab Mode and reproducibility review.
...
| Installable package | PyPI `qs-dmss==0.5.0`, release wheel, Trusted Publishing, fresh-install smoke | Ready after release | `v0.5.0` ships Lab Mode guided interpretation and guided comparison; avoid further package churn unless feedback exposes a blocker |
...
- References for related software, scientific context, the Zenodo concept DOI,
  and the current release DOI once the `v0.5.0` archive is minted.
